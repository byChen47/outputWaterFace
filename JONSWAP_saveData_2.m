...
f_min = 0.023474178; % 最小频率（Hz）
f_max = 0.211267606; % 最大频率（Hz）
% 初始化频率和谱值数组
...
writetable(wave_data, 'Wave_Time_Series.xlsx');
disp('波浪时历数据已保存到 Wave_Time_Series.xlsx 文件中。');

% 将频谱数据（Hz）保存到 Excel 文件
spectrum_data_Hz = table(f', s_f', 'VariableNames', {'Frequency_Hz', 'Spectral_Density_m2PerHz'});
writetable(spectrum_data_Hz, 'Spectrum_Hz.xlsx');
disp('频谱数据（Hz）已保存到 Spectrum_Hz.xlsx 文件中。');
...
writetable(spectrum_data_rad, 'Spectrum_radPerS.xlsx');
disp('频谱数据（rad/s）已保存到 Spectrum_radPerS.xlsx 文件中。');
